...
提供された「Python自動集計による構造化統計データ」に基づき、分析対象日の【翌日（明日）：{tomorrow_date}】における推奨狙い台（計10台）を定量スコアリングの上で決定してください。

目的は、分析対象日の【翌日（明日）：{tomorrow_date}】における高設定投入予想の精度を極限まで高めることです。
...
- 店舗の特定イベント日は【1がつく日（1日、11日、21日、31日）】です（Excel⚙️特定日末尾設定：{specific_days_rule}）。
- 分析対象日（本日：{target_date}）の【翌日（明日）：{tomorrow_date}】の高設定予想・狙い目台10台（ジャグラー5台＋スマスロ/その他5台）を決定してください。
- 本日が1の付く特定日（4/1等）の場合、翌日（4/2）は「特定日からの据え置き・お詫び上げ・隣スライド」を中心に選定すること。
- 本分テキスト内や予想根拠内に、未来の特定日（例：4月6日等）を狙い日付として絶対に記載しないこと。
...
<body class="min-h-screen p-4 md:p-8">
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="p-4 md:p-6 glass rounded-2xl text-center md:text-left">
                <h3 class="text-slate-400 text-xs font-semibold uppercase tracking-wider" id="stat-card4-title">AI予測 勝率</h3>
                <p id="stat-ai-accuracy" class="text-xl md:text-2xl font-bold mt-2 text-emerald-400">- %</p>
            </div>
        </div>
    ...
    <script>
        ...
        function updateDashboardForDate(targetDate) {{
            try {{
                ...
                // Update 4th Stat card (Cumulative Total AI Accuracy up to targetDate - S/A Ranks Only!)
                function isSARank(reasonStr) {{
                    if (!reasonStr) return false;
                    const s = reasonStr.toUpperCase();
                    const isS = s.includes('Sランク') || s.includes('推奨S') || s.includes('【S') || s.includes('[S');
                    const isA = s.includes('Aランク') || s.includes('推奨A') || s.includes('【A') || s.includes('[A');
                    const isB = s.includes('Bランク') || s.includes('推奨B') || s.includes('【B') || s.includes('[B');
                    if (isB && !isS && !isA) return false;
                    return isS || isA;
                }}

                const dedupedPreds = deduplicatePredictions(rawPredictions);
                const cumulativePreds = dedupedPreds.filter(p => p.date <= targetDate && (p.result === '〇' || p.result === '×'));
                const saPreds = cumulativePreds.filter(p => isSARank(p.reason));
                
                if (saPreds.length > 0) {{
                    const totalPredsCount = saPreds.length;
                    const wins = saPreds.filter(p => p.result === '〇').length;
                    const accuracyRaw = (wins / totalPredsCount) * 100;
                    const accuracyStr = (accuracyRaw % 1 === 0) ? accuracyRaw.toFixed(0) : accuracyRaw.toFixed(1);
                    safeSetText('stat-card4-title', 'AI予測 累計勝率 (S・A枠)');
